...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase().groupBy((c: Char) => c).toList.sortBy(_._1).map((c: (Char, String)) => (c._1, c._2.length))
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    x.foldLeft(List[(Char, Int)]()){ (result: List[(Char, Int)], occ: (Char, Int)) =>
      y.find(c => c._1 == occ._1).map{ it =>
        if(it._2 == occ._2){
          result
        } else {
          result :+ (it._1, occ._2 - it._2)
        } 
      }.getOrElse {
        result :+ occ
      }
